...
if (!isset($_SESSION['user_id']) || !isset($_SESSION['role']) || $_SESSION['role'] !== 'co') {
...
if ($application_id > 0) {
    try {
        ...
        // Get application assigned to this specific CO - FIXED: Better error handling
        $sql = "SELECT a.*, e.name, e.emp_number, e.dob, e.category, 
                ...
                WHERE a.id = ? AND e.category = 'non_gazetted'
                LIMIT 1";
        ...
        if ($stmt) {
            ...
            if ($result && $result->num_rows === 1) {
                ...
                // Check if this CO has access to this application
                if ($application['current_status'] !== 'co_pending' && 
                    $logged_in_co_email !== '' && 
                    $application['co_name'] !== null) {
                    // For already reviewed applications, check if this CO reviewed it
                    $co_check_sql = "SELECT co.email FROM controlling_officers co WHERE co.id = ?";
                    $co_check_stmt = $conn->prepare($co_check_sql);
                    $co_check_stmt->bind_param('i', $application['controlling_officer_id']);
                    $co_check_stmt->execute();
                    $co_check_result = $co_check_stmt->get_result();
                    
                    if ($co_check_result && $co_check_result->num_rows > 0) {
                        $co_check_data = $co_check_result->fetch_assoc();
                        if ($co_check_data['email'] !== $logged_in_co_email) {
                            $error = 'This application is not assigned to you.';
                            $application = null;
                        }
                    }
                    $co_check_stmt->close();
                } elseif ($application['current_status'] === 'co_pending') {
                    // For pending applications, check assignment
                    if ($logged_in_co_email !== '' && $application['co_name'] !== null) {
                        $co_check_sql = "SELECT co.email FROM controlling_officers co WHERE co.id = ?";
                        $co_check_stmt = $conn->prepare($co_check_sql);
                        $co_check_stmt->bind_param('i', $application['controlling_officer_id']);
                        $co_check_stmt->execute();
            ...
            try {
                ...
                if ($stmt) {
                    ...
                    if ($stmt->execute() && $stmt->affected_rows > 0) {
                        ...
                        // Log the action with CO name
                        $co_name = $_SESSION['full_name'] ?? $_SESSION['username'] ?? 'Unknown CO';
                        $log_entry = date('Y-m-d H:i:s') . " - CO " . $co_name . " " . $action . "d application ID: " . $application_id . " (HRMS: " . $application['hrms_id'] . ")\n";
        ...
        <div class="container-fluid">
            ...
            <div class="navbar-nav ms-auto">
                <span class="navbar-text me-3">
                    Welcome, <?php echo htmlspecialchars($_SESSION['full_name'] ?? 'Controlling Officer'); ?>
                </span>
                <!-- FIXED: Correct logout path -->

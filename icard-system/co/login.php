...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($username) || empty($password)) {
        ...
        try {
            // FIX: Get database connection properly
            $conn = getDbConnection();
            
            // Check system_users table for CO
            $sql = "SELECT id, username, password_hash, role, full_name, email, is_active FROM system_users WHERE username = ? AND role = 'co' AND is_active = 1";
            ...
            if ($result && $result->num_rows === 1) {
                ...
                if (password_verify($password, $user['password_hash'])) {
                    ...
                    $_SESSION['full_name'] = $user['full_name'];  // Changed from 'user_name'
                    $_SESSION['username'] = $user['username'];
                    $_SESSION['email'] = $user['email'];  // Added email for dashboard

    ...
    if (empty($errors)) {
        try {
            ...
            if (move_uploaded_file($_FILES['photo']['tmp_name'], $photo_dir . $photo_filename) &&
                ...
                if ($stmt) {
                    $stmt->bind_param('ssssisssssisssssss', 
                        $hrms_id, $designation, $ticket_number, $office_shop, $controlling_officer_id,
                        $department_id, $date_of_appointment, $date_of_joining, $date_of_retirement,
                        $blood_group, $height, $mobile_number, $identification_mark, $address_line1,
                        $address_line2, $email, $photo_filename, $sig_filename, $remarks
                    );
                    
                    if ($stmt->execute()) {
                        $success = true;
                        $success_message = "Your I-Card application has been submitted successfully!";
                    } else {
...
<body class="bg-light">
                    ...
                    <div class="form-section">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control" name="remarks" rows="3" 
                                      placeholder="Any additional information for CO, Dealer, and AWO (Optional)"><?php echo htmlspecialchars($_POST['remarks'] ?? ''); ?></textarea>
                            <div class="form-text">This will be visible to CO, Dealer, and AWO during review process</div>
                        </div>
                    </div>
    ...
    <script>
        ...
        const isGazetted = <?php echo $is_gazetted ? 'true' : 'false'; ?>;

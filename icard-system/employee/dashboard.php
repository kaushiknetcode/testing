...
try {
    ...
    if (!$has_icard) {
        $app_sql = "SELECT * FROM applications WHERE hrms_id = ? AND current_status != 'rejected' ORDER BY created_at DESC LIMIT 1";
        ...
        if ($app_stmt) {
            ...
            if ($app_result && $app_result->num_rows > 0) {
                $pending_application = $app_result->fetch_assoc();
                
                // If application is approved, there might be an I-Card generated
                if ($pending_application['current_status'] === 'approved') {
                    // Re-check for I-Card (might have been just generated)
                ...
                // No pending applications - show apply button
                    ...
                    <div class="dashboard-card card mb-4">
                        ...
                        <div class="card-body">
                            <div class="row align-items-center">
                                <div class="col-md-8">
                                    ...
                                    <h4>
                                        <span class="badge bg-<?php echo getStatusBadgeClass($pending_application['current_status']); ?> status-badge">
                                            <?php echo getReadableStatus($pending_application['current_status']); ?>
                                        </span>
                                    </h4>
                                    <p class="text-muted mb-0">
                                        <i class="fas fa-calendar me-1"></i>
                                ...
                                <div class="col-md-4 text-md-end">
                                    <?php if ($pending_application['current_status'] === 'draft'): ?>
